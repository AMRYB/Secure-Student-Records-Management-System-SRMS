...
async function recordAttendance(){
  ...
  if (!student_id || !course_id){
    return setMsg(recordMsg, "Please enter valid StudentID and CourseID.");
  }

  const res = await fetch("/api/instructor/attendance/record", {
    method: "POST",
  ...
  setMsg(recordMsg, "Attendance recorded ", true);
...
async function loadAttendance(){
  ...
  const res = await fetch(`/api/instructor/attendance?${params.toString()}`, { credentials: "include" });
  const data = await res.json().catch(() => ({}));
